...
Based on v02 configuration as baseline
"""
...
def train_and_evaluate(
    ...
    seed: int = 42,
    verbose: bool = False,
    trial: Optional[Trial] = None,  # pruning용
) -> Dict[str, float]:
    """모델 학습 및 평가, 지표 반환"""
...
def create_objective(
    ...
    def objective(trial: Trial) -> float:
        ...
        # 증강 파라미터 (핵심 탐색 대상)
        aug_config = AugmentConfig(
            max_time_shift=trial.suggest_int('max_time_shift', 3, 6),
            ret_scale_jitter_std=trial.suggest_float('ret_scale_jitter_std', 0.02, 0.10),
            ret_noise_std=trial.suggest_float('ret_noise_std', 0.003, 0.015),
            ret_drop_prob=trial.suggest_float('ret_drop_prob', 0.02, 0.12),
            vol_noise_std=trial.suggest_float('vol_noise_std', 0.002, 0.012),
            vol_drop_prob=trial.suggest_float('vol_drop_prob', 0.0, 0.08),
            clamp_ret=0.25,
            clamp_vol=1.0,
        )
        ...
        ema_m = trial.suggest_float('ema_m', 0.990, 0.999)
        ...
        try:
            metrics = train_and_evaluate(
                ...
                lr=lr,
                epochs=epochs,
                max_tickers_train=400,
                max_tickers_eval=700,
                device=device,
                seed=42,
                verbose=False,
                trial=trial,
        ...
        # 복합 점수
        score = (
            overlap_ret * 0.6 +          # 수익률 중첩 (가장 중요)
            overlap_vol * 0.3 +          # 변동성 중첩
            min(pr / 10, 0.1) -          # PR 보너스 (적당한 압축)
            pr_penalty -
            cos_penalty
        )
...
def run_optimization(
    ...
    study_name: str = "byol_v02_optimization",
    n_trials: int = 50,
...
def train_final_model(
    ...
    if save_path is None:
        ...
        save_path = f"byol_v02_optuna_{timestamp}.pt"
    ...
    dl = DataLoader(
        ds, batch_size=params['batch_size'], shuffle=True,
        num_workers=4, drop_last=True,
        worker_init_fn=seed_worker, generator=g,
        pin_memory=(device == 'cuda'),
        persistent_workers=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--study_name', type=str, default='byol_v02_tuning')
